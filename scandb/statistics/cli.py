...
from scandb.statistics.queries import get_scan_stats, get_port_stats, get_host_port_list, get_vuln_stats
from scandb.report.cli import write_to_template


def handle_scan_stats(db, outfile, write_file = False, delimeter= ";", gen_docx=False, template=""):
    results_csv = []
    results_cli = []
    ...
    for s in stats:
        ...
        results_csv.append(delimeter.join([stat.id, stat.start, stat.end, stat.elapsed, stat.hosts_total, stat.hosts_up,
                                      stat.hosts_down, stat.name]))
...
def handle_vuln_stats(db, outfile, write_file = False, delimeter=";", gen_docx=False, template=""):
    results_csv = []
    results_cli = []
    ...
    for s in stats:
        ...
        results_csv.append(delimeter.join([s.address, s.critical, s.high, s.medium, s.low, s.info]))
...
def handle_port_stats(db, outfile, write_file = False, delimeter=";", gen_docx=False, template=""):
    results_csv = []
    results_cli = []
    fmt = '{0:>20}{1:>15}{2:>15}'
    ...
    for s in stats:
        ...
        results_csv.append(delimeter.join([s.address, s.tcp, s.udp]))
...
def handle_host_port_list(db, outfile, delimiter=";", gen_docx=False, template=""):
    ...
    with open(filename,'w') as f:
        for r in rows:
            f.write(delimiter.join([r.address, r.ports, r.protocol]))
            f.write("\n")
...
def statistics_cli():
    ...
    if args.scan_statistics:
        handle_scan_stats(args.db, args.outfile, args.write_file, args.delimeter, args.docx, args.template)

    if args.vuln_statistics:
        handle_vuln_stats(args.db, args.outfile, args.write_file, args.delimeter, args.docx, args.template)

    if args.port_statistics:
        handle_port_stats(args.db, args.outfile, args.write_file, args.delimeter, args.docx, args.template)
